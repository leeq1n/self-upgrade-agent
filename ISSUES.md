...
| v1.8.0-alpha (2026-07-02) | `python -m self_upgrade <subcommand>` 统一 CLI (run/evolve/status/unlock/cull);`src/learning.py` + `learning.db` (attempts/blacklist/seen_papers/convergence_state/auto_blacklist);C8-C12 不变性;Phase A 部分完成 (A1+A4 落地);3 round live 跑出 0 进展 (commit 02dc574) — ModelScope 网关 8 keys 全 401/403/timeout。**未 tag v1.8.0** (等 Phase A 完整 + Phase B 至少 B1) |
