...
- **Snapshot files**: created ad-hoc in
  ...
  `~/AppData/Local/Temp/sua-snapshot-<topic>-<date>.md`.

  Examples observed:

  - `sua-snapshot-sua-onboarding-20260713.md` (4.6KB,

    103 lines)

  - `sua-snapshot-self-upgrade-agent-20260713.md`
...
5. **Cross-session search**: integration with session_search
   ...
   'sua-snapshot' tag" or "search for 'snapshot-{topic}'").
...
- **Tier 1 (ephemeral, default)**: `~/AppData/Local/Temp/sua-snapshot-<topic>-<date>.md`
...
- **Tier 2 (persistent, opt-in)**: `<project>/agent-tools/snapshots/<topic>-<date>.md`
...
1. **Locate snapshot**: `find . -name "sua-snapshot-*.md"` (Tier 2)

   or `ls ~/AppData/Local/Temp/sua-snapshot-*.md` (Tier 1)
...
- `session_search(query="sua-snapshot", limit=10)` — find all snapshots by topic
